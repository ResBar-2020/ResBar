...
var vm = new Vue({
  ...
  methods: {
    ...
    ordenar: function () {
      ...
      if (this.ascendente == true) {
        if (this.activos) {
          axios
            .get(this.uri + "?filter[order][0]=mesa ASC")
            .then((response) => {
              this.ordenes = response.data;
            })
            .catch((e) => {
        ...
        if (this.activos) {
          axios
            .get(this.uri + "?filter[order][0]=mesa DESC")
            .then((response) => {
              this.ordenes = response.data;
            })
            .catch((e) => {
    ...
    ordenarMesero: function () {
      ...
      if (this.ascendente == true) {
        if (this.activos) {
          axios
            .get(this.uri + "?filter[order][0]=mesero ASC")
            .then((response) => {
              this.ordenes = response.data;
            })
            .catch((e) => {
        ...
        if (this.activos) {
          axios
            .get(
              this.uri +
                "?filter=%7B%0A%20%20%22order%22%3A%20%5B%0A%20%20%20%20%22mesero%20DESC%22%0A%20%20%5D%0A%7D"
            )
            .then((response) => {
              this.ordenes = response.data;
            })
          ...
          axios
            .get(
              this.uri +
                "?filter=%7B%0A%20%20%22order%22%3A%20%5B%0A%20%20%20%20%22mesero%20DESC%22%0A%20%20%5D%2C%0A%20%20%22where%22%3A%20%7B%0A%20%20%20%20%22estado%22%3A%20%22A%22%0A%20%20%7D%0A%7D"
            )
            .then((response) => {
              this.lactivos = response.data;
            })
      ...
      if (this.ascendente == true) {
        if (this.activos) {
          axios
            .get(
              this.uri +
                "?filter=%7B%0A%20%20%22order%22%3A%20%5B%0A%20%20%20%20%22cliente%20ASC%22%20%0A%20%20%5D%0A%7D"
            )
            .then((response) => {
              this.ordenes = response.data;
            })
          ...
          axios
            .get(
              this.uri +
                "?filter=%7B%0A%20%20%22order%22%3A%20%5B%0A%20%20%20%20%22cliente%20ASC%22%0A%20%20%5D%2C%0A%20%20%22where%22%3A%20%7B%0A%20%20%20%20%22estado%22%3A%20%22A%22%0A%20%20%7D%0A%7D"
            )
            .then((response) => {
              this.lactivos = response.data;
            })
        ...
        if (this.activos) {
          axios
            .get(
              this.uri +
                "?filter=%7B%0A%20%20%22order%22%3A%20%5B%0A%20%20%20%20%22cliente%20DESC%22%0A%20%20%5D%0A%7D"
            )
            .then((response) => {
              this.ordenes = response.data;
            })
          ...
          axios
            .get(
              this.uri +
                "?filter=%7B%0A%20%20%22order%22%3A%20%5B%0A%20%20%20%20%22cliente%20DESC%22%0A%20%20%5D%2C%0A%20%20%22where%22%3A%20%7B%0A%20%20%20%20%22estado%22%3A%20%22A%22%0A%20%20%7D%0A%7D"
            )
            .then((response) => {
              this.lactivos = response.data;
            })
      ...
      if (this.ascendente == true) {
        if (this.activos) {
          axios
            .get(
              this.uri +
                "?filter=%7B%0A%20%20%22order%22%3A%20%5B%0A%20%20%20%20%22id%20ASC%22%20%0A%20%20%5D%0A%7D"
            )
            .then((response) => {
              this.ordenes = response.data;
            })
          ...
          axios
            .get(
              this.uri +
                "?filter=%7B%0A%20%20%22order%22%3A%20%5B%0A%20%20%20%20%22id%20ASC%22%0A%20%20%5D%2C%0A%20%20%22where%22%3A%20%7B%0A%20%20%20%20%22estado%22%3A%20%22A%22%0A%20%20%7D%0A%7D"
            )
            .then((response) => {
              this.lactivos = response.data;
            })
        ...
        if (this.activos) {
          axios
            .get(
              this.uri +
                "?filter=%7B%0A%20%20%22order%22%3A%20%5B%0A%20%20%20%20%22id%20DESC%22%0A%20%20%5D%0A%7D"
            )
            .then((response) => {
              this.ordenes = response.data;
            })
          ...
          axios
            .get(
              this.uri +
                "?filter=%7B%0A%20%20%22order%22%3A%20%5B%0A%20%20%20%20%22id%20DESC%22%0A%20%20%5D%2C%0A%20%20%22where%22%3A%20%7B%0A%20%20%20%20%22estado%22%3A%20%22A%22%0A%20%20%7D%0A%7D"
            )
            .then((response) => {
              this.lactivos = response.data;
            })
    ...
    mostrarActivos: function () {
      axios
        .get(
          this.uri +
            "?filter=%7B%0A%20%20%22where%22%3A%20%7B%0A%20%20%20%20%22estado%22%3A%20%22A%22%0A%20%20%7D%0A%7D"
        )
        .then((response) => {
          this.lactivos = response.data;
        })
        ...
        if (res.length != 0) {
          if (res[0].rol == "admin") {
            if (this.action == "eliminar") {
              ...
              window.location =
                "./" + this.action + ".html?id=" + this.ordenSelected.id;
            }
          } else {
            document.getElementById("errorMsg").classList.add("text-danger");
            document.getElementById("errorMsg").textContent =
              "El pin debe ser de un administrador";
          ...
          document.getElementById("errorMsg").textContent =
            "Por favor ingrese un pin valido";
        }
      }
    },
    ...
    eliminarOrden: function () {
      ...
      if (texto == "") {
        ...
        //Elimina la orden
        axios
          .delete(this.uri + "/" + this.ordenSelected.id)
          .then(function (res) {
    ...
    setProdImprimir: function () {
      getProductsFromOrder(localStorage.idOrdenImprimir).then((response) => {
        ...
        document.getElementById("horaImpProd").innerHTML =
          "Hora: " +
          new Date(response.fecha)
            .toTimeString()
            .replace(/.*(\d{2}:\d{2}:\d{2}).*/, "$1");
        if (
          this.imprimirProd.detalleOrden.filter(
            (e) => e.preparado === true && e.cantidad > 0
          ).length > 0 ||
          this.imprimirProd.domicilio === true
        ) {
          window.print();
        } else {
  ...
  mounted() {
    this.getParametros().then((res) => {
      if (
        localStorage.estado === "nuevo" &&
        localStorage.idOrdenImprimir &&
        vm.parametros[6].valor === "true"
      ) {
        this.editarOrdenImp = false;
        this.setProdImprimir();
      } else if (
        localStorage.estado === "editar" &&
        localStorage.idOrdenImprimir &&
        vm.parametros[6].valor === "true"
      ) {
