<template>
  <div>
    <div v-for="idioma in idiomas" :key="idioma._id">
      <header-dashboard
        :title="idioma.views[0].labels.title"
        :subtitle="idioma.views[0].labels.subtitle"
      ></header-dashboard>
      <v-row>
        <v-col cols="4" offset="1">
          <v-text-field
            :label="idioma.views[0].labels.search"
        ...
        <v-col cols="4" offset="1">
          ...
          >
            <v-btn elevation="13" outlined class="primary white--text my-3"
      ...
      <v-container fluid>
        <v-row>
          <v-col
            v-for="(orden, index) in listaDomicilios"
            :key="index"
            v-show="filtro(index)"
          >
            <template>
              <v-card max-width="374" hover height="390" elevation="17">
                <v-banner
                  ...
                  sticky
                  :class="{
                    'primary lighten-3': orden.domicilioEtapa === 0,
                    primary: orden.domicilioEtapa === 1,
                    'primary darken-4': orden.domicilioEtapa === 2,
                  }"
                >
                  {{
                    orden.domicilioEtapa === 0
                      ? idioma.views[0].labels.state.todo
                      : orden.domicilioEtapa === 1
                      ? idioma.views[0].labels.state.inprocess
                      : idioma.views[0].labels.state.complete
                  }}
                </v-banner>
                <v-card class="overflow-y-auto pa-2 scroll-hide" height="260">
                  ...
                  <div>
                    ...
                    <div>
                      {{
                        idioma.views[0].labels.direction +
                          ":" +
                  ...
                  <v-footer absolute class="font-weight-medium">
                    <v-chip-group class="col">
                      <v-chip
                        ...
                        color="teal"
                        text-color="white"
                        @click="completarEtapa(orden)"
                        :disabled="orden.domicilioEtapa === 2"
                        dark
                      >
                        <v-avatar left>
...
export default {
  ...
  methods: {
    ...
    filtro(valor_orden) {
      ...
      let cad =
        this.listaDomicilios[valor_orden]._id +
    ...
    async completarEtapa(orden) {
      if (orden.domicilioEtapa === 1 && !orden.cobrada) {
        ...
        orden.domicilioEtapa += 1;
        await this.modificarEtapaDomicilio(orden);
        this.changeLista();
    ...
    async changeLista() {
      ...
      if (!this.showComplete) {
        this.listaDomicilios = this.ordenes.filter(
          (orden) => orden.domicilioEtapa !== 2
        );
      } else {
